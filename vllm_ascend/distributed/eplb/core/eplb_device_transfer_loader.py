...
class D2DExpertWeightLoader(BaseLoader):
    ...
    @override
    def prepare_send(self, expert_send_info):
        for send_info in expert_send_info:
            ...
            local_expert_id = self.eplb_adaptor.expert_map_per_layer_cpu[
                self.layer_id][global_expert_id_to_send].item()
            for src_tensor in self.eplb_adaptor.expert_param_per_layer[
                self.layer_id][local_expert_id]:
                self.comm_op_list.append(
                    dist.P2POp(dist.isend, src_tensor, dst_rank))
    
    @override
    def prepare_recv(self, expert_recv_info):
        buffer_tensor_id = 0
    ...
    def generate_expert_d2d_transfer_task(self, expert_send_info,
        ...
        self.prepare_send(expert_send_info)

        self.prepare_recv(expert_recv_info)
